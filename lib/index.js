...
exports.init = function (sequelize, optionsArg) {
  ...
  var afterHook = function afterHook(instance, opt) {
    ...
    if (instance.context && instance.context.delta && instance.context.delta.length > 0) {
      ...
      const source = ns.get(options.continuationKey) || opt[options.continuationKey];
      ...
      // Save revision
      return revision.save({ transaction }).then(function (revision) {
        if (options.enableRevisionChangeModel) {
          return Promise.all(_.map(delta, function (difference) {
            ...
            return d.save({ transaction }).then(function (d) {
              // Add diff to revision
              return revision['add' + helpers.capitalizeFirstLetter(options.revisionChangeModel)](d, { transaction });
            }).catch(function (err) {
