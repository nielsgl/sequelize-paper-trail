...
let failHard = false;
let hasWarnedNodeDeprecation = false;
...
const shouldWarnNodeDeprecation = () => {
	if (process.env.SUPPRESS_NODE_DEPRECATION === '1') {
		return false;
	}
...
const warnDeprecatedNode = () => {
	if (hasWarnedNodeDeprecation || !shouldWarnNodeDeprecation()) {
		return;
	}
	hasWarnedNodeDeprecation = true;
	// eslint-disable-next-line no-console
	console.warn(
		`sequelize-paper-trail: Node <${MIN_NODE_MAJOR} is deprecated and will be unsupported in the next major release. ` +
			'Please upgrade to Node 20+ or set SUPPRESS_NODE_DEPRECATION=1 to silence this warning.',
	);
};
