...
- Current phase: `Step 5 verification completed (post-ship-gate revision)`
- Plan Gate: `Approved`
- Ship Gate: `Approved`

## Scope

Update documentation and release policy artifacts for the v4 bridge contract (Node >=20, Sequelize v5 + v6, bugfix-only release line), while explicitly deferring release-note/`CHANGELOG` entries to `PRD-002 WI-004`.
...
- [x] Expand v4 bridge documentation with substantive contract details (support matrix/checklist/operating policy), not only sequencing notes.
...
- Mitigation: explicitly document `CHANGELOG` ownership in WI-004 follow-up notes inside release-policy/checklist artifacts.
...
- Action taken: expanded WI-002 content across README/MIGRATION/RELEASE-POLICY/RELEASE-CHECKLIST and re-ran verification.
- Current state: pending renewed ship approval.
