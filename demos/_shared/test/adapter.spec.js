...
const hasAdapters = (() => {
	try {
		require.resolve('sequelize-paper-trail/dist/adapters/sequelize-v5');
		return true;
	} catch (err) {
		return false;
	}
...
const sequelizePaperTrail = (() => {
	try {
		return require('sequelize-paper-trail');
	} catch (err) {
		return null;
	}
})();

const describeAdapters = hasAdapters ? describe : describe.skip;
const describeAdapterHelpers =
	sequelizePaperTrail &&
	typeof sequelizePaperTrail._resolveSequelizeMajor === 'function'
		? describe
		: describe.skip;

const describeAdapterLoader =
	sequelizePaperTrail && typeof sequelizePaperTrail._createAdapter === 'function'
		? describe
		: describe.skip;

describeAdapters('sequelize adapter (v5)', () => {
	it('exposes the adapter surface needed by core', async () => {
		const sequelize = createSequelize();
		const adapter = loadAdapter(
			'sequelize-paper-trail/dist/adapters/sequelize-v5',
		)(sequelize);
	...
	it('uses cls-hooked when SEQUELIZE_CLS is set', async () => {
		jest.isolateModules(() => {
			...
			jest.doMock(
				'sequelize-paper-trail/dist/adapters/cls-hooked',
				() => () => ({
				getNamespace,
				createNamespace,
			}),
			...
			const createAdapter = loadAdapter(
				'sequelize-paper-trail/dist/adapters/sequelize-v5',
			);
			const sequelize = createSequelize();
...
describeAdapters('sequelize adapter (v6)', () => {
	it('exposes the adapter surface needed by core', async () => {
		...
		const adapter = loadAdapter(
			'sequelize-paper-trail/dist/adapters/sequelize-v6',
		)(sequelize);
...
describeAdapterHelpers('adapter selection helper', () => {
	const { _resolveSequelizeMajor } = require('sequelize-paper-trail');
	let originalEnv;
...
describeAdapterLoader('createAdapter loader', () => {
	...
	it('selects the v5 adapter when forced via env', () => {
		jest.isolateModules(() => {
			...
			const { _createAdapter } = require('sequelize-paper-trail');
			_createAdapter({
...
describeAdapters('cls-hooked error handling', () => {
	it('throws when cls-hooked is missing in v6', async () => {
		await jest.isolateModules(async () => {
			jest.resetModules();
			const getClsHooked = require(
				'sequelize-paper-trail/dist/adapters/cls-hooked',
			);
